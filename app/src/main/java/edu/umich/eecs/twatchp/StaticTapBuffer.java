 ...
 */
public class StaticTapBuffer extends TapBuffer {
    private byte [] buffer = new byte[20*44100];
    ...
    public StaticTapBuffer (String name) {
        this.name = name;
    }
    ...
    public synchronized int buffer_handle (String action, byte[] array, int length) {
        ...
        if (action.equals("add")) {
            ...
            for (int i = 0; i < saved; i++)  buffer[tail + i] = array[i];
            ...
            for (int i = 0; i < length; i++) array[i] = buffer[head + i];
            head += length;
            saved = length;
